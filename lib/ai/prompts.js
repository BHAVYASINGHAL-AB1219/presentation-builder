...
6. CRITICAL: Do NOT treat the layout list as a checklist. You do NOT need to use every layout type. Only use a layout if it genuinely fits the content. For example, only use BIG_NUMBER if the topic has a genuinely impressive statistic, and only use QUOTE if there is a real, meaningful quote to display.
...
- Use real-world examples and data when they genuinely support the message
...
${outline.slides
  ...
  Section: ${s.section}
  Key ideas: ${s.talking_points.join("; ")}`
  )
  .join("\n\n")}

For EACH slide, you must decide the best layout_type based on the actual content you write. Choose layouts that genuinely match what is being communicated — do NOT follow a fixed sequence.`;
}
